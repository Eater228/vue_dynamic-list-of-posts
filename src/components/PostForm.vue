...
const handleSave = async () => {
  ...
  if (selectedPost) {
    updatePost({ ...selectedPost, title: title.value, body: postBody.value }).then((res) => {
      emit('updatePost', res.data)
      clearForm()
    })
  } else {
    addNewPost({ userId, title: title.value, body: postBody.value }).then((res) => {
  ...
  <div class="content">
    ...
    <form @submit.prevent="handleSave">
      ...
      <div class="field is-grouped">
        ...
        <div class="control">
          <button type="button" @click="handleRestore" class="button is-link is-light">
            Cancel
          </button>
        </div>
      </div>
